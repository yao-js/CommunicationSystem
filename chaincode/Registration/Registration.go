...
func (s *SmartContract) createRegistration(APIstub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	register.Uid = args[0]
	register.DeviceOwnerPriHex = args[1]
	register.RText = args[2]
	register.SText = args[3]
	register.DeviceCert = args[4]
	register.DeviceEcdhPub = args[5]
	...
	//如不存在则添加到blockchian中
	registerAsBytes, _ := json.Marshal(register)
