        ...
        if (StringUtils.isBlank(configFile)) {
            ...
            if (!StringUtils.isBlank(propertiesKey) && System.getProperties().containsKey(propertiesKey)) {
                ...
                if (!clz.isAnnotationPresent(PropertySource.class)) {
                    log.error("use @PropertySource to set default configuration file name","aa");
                    return false;
                }

                log.info("get source file from @PropertySource", "aa");
                ...
                if (!files[0].startsWith("classpath:")) {
                    log.error("configuration file must be in classpath", "aa");
                    return false;
                }
                file = files[0];
            }
