...
public class Ecdsa {
    ...
    public static Signature sign(String message, PrivateKey privateKey, MessageDigest hashfunc) {
        ...
        Point randomSignPoint = java.lang.Math.multiply(curve.G, randNum, curve.N, curve.A, curve.P);
        BigInteger r = randomSignPoint.x.mod(curve.N);
        BigInteger s = ((numberMessage.add(r.multiply(privateKey.secret))).multiply(java.lang.Math.inv(randNum, curve.N))).mod(curve.N);
    ...
    public static boolean verify(String message, Signature signature, PublicKey publicKey, MessageDigest hashfunc) {
        ...
        BigInteger w = java.lang.Math.inv(s, curve.N);
        Point u1 = java.lang.Math.multiply(curve.G, numberMessage.multiply(w).mod(curve.N), curve.N, curve.A, curve.P);
        Point u2 = java.lang.Math.multiply(publicKey.point, r.multiply(w).mod(curve.N), curve.N, curve.A, curve.P);
